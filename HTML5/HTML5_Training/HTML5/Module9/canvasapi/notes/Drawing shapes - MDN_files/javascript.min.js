...
WT.tv="8.6.2";WT.dl="0";WT.ssl=(window.location.protocol.indexOf('https:')==0)?"1":"0";DCS.dcsdat=dCurrent.getTime();DCS.dcssip=window.location.hostname;DCS.dcsuri=window.location.pathname;WT.es=DCS.dcssip+DCS.dcsuri;if(window.location.search){DCS.dcsqry=window.location.search;}
...
WebTrends.prototype.dcsCollect=function(){if(this.enabled){this.dcsVar();this.dcsMeta();this.dcsAdv();this.dcsTag();}}};function dcsMultiTrack(){if(typeof(_tag)!="undefined"){return(_tag.dcsMultiTrack());}}
...
if('jQuery'){$(document).ready(function(){$('input[placeholder]').placeholder();$("body").addClass("hasJS");$("#nav .toggle").click(function(){$("#nav .sub-menu:visible").slideUp(100).attr("aria-hidden","true");$("#nav .open").removeClass("open");$(this).siblings(".sub-menu").slideToggle(150).removeAttr("aria-hidden");$(this).toggleClass("open");return false;});$("#nav .sub-menu").hover(function(){$(this).show().removeAttr("aria-hidden");},function(){$(this).delay(100).slideUp(150).attr("aria-hidden","true");$("#nav .toggle").delay(100).removeClass("open").blur();});$(document).bind('click',function(e){var $clicked=$(e.target);if(!$clicked.parents().hasClass("menu"))
$("#nav .sub-menu").hide().attr("aria-hidden","true");$("#nav .toggle").removeClass("open");});$("a, input, textarea, button, :focus").bind('focus',function(e){var $focused=$(e.target);if(!$focused.parents().hasClass("menu"))
$("#nav .sub-menu").hide().attr("aria-hidden","true");$("#nav .toggle").removeClass("open");});$("#tools, .user-state").superfish({delay:200,animation:{height:'show'},speed:100,autoArrows:false,dropShadows:false});$(".menu .toggle").click(function(){return false;});$("li.menu").mouseover(function(){$(this).addClass('sfhover');});$("li.menu").mouseout(function(){$(this).removeClass('sfhover');});var search=$('#site-search,#doc-search');search.attr('action',search.attr('data-url')).removeAttr('data-url').children('input[name=sitesearch]').remove();});jQuery.fn.placeholder=function(new_value){if(new_value){this.attr('placeholder',new_value);}
if('placeholder'in document.createElement('input')){return this;}
if(new_value&&this.hasClass('placeholder')){this.val('').blur();}
return this.focus(function(){var $this=$(this),text=$this.attr('placeholder');if($this.val()==text){$this.val('').removeClass('placeholder');}}).blur(function(){var $this=$(this),text=$this.attr('placeholder');if($this.val()==''){$this.val(text).addClass('placeholder');}}).each(function(){var self=$(this);self.closest('form').submit(function(){if(self.hasClass('placeholder')){self.val('');}});}).blur();};}